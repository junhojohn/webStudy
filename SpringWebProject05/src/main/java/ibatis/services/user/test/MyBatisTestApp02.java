...
public class MyBatisTestApp02 {
	...
	public static void main(String[] args) throws Exception{
		...
		List<User> list = sqlSession.selectList("userMapper02.selectUserList01","user");
		...
		list = sqlSession.selectList("userMapper02.selectUserList01","user%");
		for (int i =0 ;  i < list.size() ; i++) {
			System.out.println( "<"+ ( i +1 )+"> 번째 회원.."+ list.get(i).toString() );
		}
		System.out.println("\n");
		...
		System.out.println(":: 2. 01로 끝나는 userId User(SELECT)  ? ");
		 list = sqlSession.selectList("userMapper02.selectUserList01","01");
		for (int i =0 ;  i < list.size() ; i++) {
			System.out.println( "<"+ ( i +1 )+"> 번째 회원.."+ list.get(i).toString() );
		}
		System.out.println("\n");
		...
		list = sqlSession.selectList("userMapper02.selectUserList02","user");
		for (int i =0 ;  i < list.size() ; i++) {
			System.out.println( "<"+ ( i +1 )+"> 번째 회원.."+ list.get(i).toString() );
		}
		System.out.println("\n");
		...
		System.out.println(":: 5. 01 문자가 있는 userId User(SELECT)  ? ");
		 list = sqlSession.selectList("userMapper02.selectUserList02","01");
		for (int i =0 ;  i < list.size() ; i++) {
			System.out.println( "<"+ ( i +1 )+"> 번째 회원.."+ list.get(i).toString() );
		}
		System.out.println("\n");
	...
	selectOne("userMapper02.findUserId",
