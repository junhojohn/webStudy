		...
		if(actionPage != null) {
			if(actionPage.equals(REQ_ACTION_PAGES_ENUM_TEST.REQ_LOGON.getRequestActionKey()) 	||
				...
				if(userVO.isActive()) {
					...
					request.getServletContext().getRequestDispatcher(REQ_ACTION_PAGES_ENUM_TEST.REQ_LOGON.getJspPathURI()).forward(request, response);
					System.out.println(getClass().getName() + ".preHandle() end.");
					return false;
				}

			}else if(actionPage.equals(REQ_ACTION_PAGES_ENUM_TEST.REQ_LOGON_ACTION.getRequestActionKey())) {
				System.out.println("Logon in progress...");
				request.getServletContext().getRequestDispatcher(REQ_ACTION_PAGES_ENUM_TEST.REQ_LOGON_ACTION.getJspPathURI()).forward(request, response);
				System.out.println(getClass().getName() + ".preHandle() end.");
				return true;
			}
		}
